...
//				file_put_contents( basename( $name ), $entry_xml ); // @to do comment!!!
	...
	public function toHTML( $worksheetIndex = 0 ) {
		$s = '<table class="excel table table-bordered"><thead class="thead-dark">';
		foreach ( $this->rows( $worksheetIndex ) as $key => $r ) {
			if ($key === 0) {
				$s .= '<tr>';
				foreach ( $r as $num => $c ) {
					$s .= '<th nowrap><div class="dropup" data-col="'.$num.'"><button class="btn btn-info dropdown-toggle" type="button" data-toggle="dropdown">' . ( $c === '' ? '&nbsp' : htmlspecialchars( $c, ENT_QUOTES ) ) . '</button>
					<div class="dropdown-menu">
					<button class="btn btn-sm btn-secondary btn-asc dropdown-item" type="button">Сортировка по возрастанию (А-Я)</button>
					<button class="btn btn-sm btn-secondary btn-desc dropdown-item" type="button">Сортировка по убыванию (Я-А)</button>
					<p class="dropdown-item mb-0 mt-3"><b>Фильтр:</b></p>
					<input class="filter-input dropdown-item" type="text" placeholder="Введите значение">
					<button class="btn btn-sm btn-warning btn-clear-all dropdown-item" type="button">Очистить всё</button>
					</div></div></th>';
				}
				$s .= "</tr></thead><tbody>\r\n";
			} else {
